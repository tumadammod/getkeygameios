...
local Settings = StreeLib.Settings
local Flags = StreeLib.Flags

local SetProps, SetChildren, InsertTheme, Create do
    InsertTheme = function(Instance, Type)
        table.insert(StreeLib.Instances, {
            Instance = Instance,
            Type = Type
        })
        return Instance
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                if rawget(decode, "UISize") then StreeLib.Save["UISize"] = decode["UISize"] end
                if rawget(decode, "TabSize") then StreeLib.Save["TabSize"] = decode["TabSize"] end
                if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then StreeLib.Save["Theme"] = decode["Theme"] end
            end
        end
    end
    
    pcall(Save, "stree library v1.0.json")
end
...
local Connections, Connection = {}, StreeLib.Connection do
...
local function VerifyTheme(Theme)
    for name,_ in pairs(StreeLib.Themes) do
        if name == Theme then
            return true
        end
    end
...
local Theme = StreeLib.Themes[StreeLib.Save.Theme]

local function AddEle(Name, Func)
    StreeLib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
    local Element = StreeLib.Elements[Ele](Instance, props, ...)
    return Element
end
...
function StreeLib:GetIcon(index)
...
function StreeLib:SetTheme(NewTheme)
    ...
    for _, Val in pairs(StreeLib.Instances) do
    ...
    for _, TabData in pairs(StreeLib.Tabs) do
...
function StreeLib:SetScale(NewScale)
...
function StreeLib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "StreeLib v1.0"
    local WMiniText = Configs[2] or Configs.SubTitle or "by : StreeHub"
    ...
    local UISizeX, UISizeY = unpack(StreeLib.Save.UISize)
    ...
    local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
        Size = UDim2.new(0, StreeLib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
    ...
    ConnectSave(ControlSize1, function()
        if not Minimized then
            StreeLib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
            SaveJson("stree library v1.0.json", StreeLib.Save)
        end
    end)
    
    ConnectSave(ControlSize2, function()
        StreeLib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("stree library v1.0.json", StreeLib.Save)
    end)
    ...
    local function UpdateSearch(Query)
        ...
        for _, ElementData in pairs(StreeLib.AllElements) do
    ...
    function Window:SelectTab(TabSelect)
        if type(TabSelect) == "number" then
            StreeLib.Tabs[TabSelect].func:Enable()
        else
    ...
    function Window:MakeTab(paste, Configs)
        ...
        FirstTab = true
        local Tab = {}
        table.insert(StreeLib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
        Tab.Cont = Container
        ...
        function Tab:AddSection(Configs)
            ...
            table.insert(StreeLib.AllElements, {
                Name = SectionName,
            ...
            local Section = {}
            table.insert(StreeLib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    task.spawn(function()
        ...
        ToggleGui.Name = "StreeHub_Toggle_Circle"
...
function StreeLib:SetUIBorderColor(color)
    if typeof(color) == "Color3" then
        self.Themes[self.Save.Theme]["UI Border Color"] = color
        if self.Save.Theme == StreeLib.Save.Theme then
...
function StreeLib:SetDiscordBorderColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetProfileBorderColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetHubColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetStrokeColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetTextColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetToggleOnColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetToggleOffColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetToggleKnobOnColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetToggleKnobOffColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetToggleBorderColor(color)
    if typeof(color) == "Color3" then
...
function StreeLib:SetBorderThickness(thickness)
...
function StreeLib:GetBorderColors()
    return {
...
function StreeLib:GetToggleColors()
    return {
...
function StreeLib:ResetBorderColors()
    local defaultTheme = self.Themes.StreeHub
...
function StreeLib:ResetToggleColors()
    local defaultTheme = self.Themes.StreeHub
